...
MZNT_VulkanMesh* MZNT_UploadMesh_Vulkan(MZNT_VulkanRenderer* renderer, MZNT_MeshCreateInfo* createInfo, PNSLR_Allocator tempAllocator)
{
    ...
    for (i32 i = 0; i < createInfo->vertexAttributes.count; i++)
    {
        MZNT_VertexAttribute* attr = &(createInfo->vertexAttributes.data[i]);
        singleVertexSize += (attr->componentCount * sizeof(f32));
    }

    i64 vertexBufferSize = singleVertexSize * createInfo->vertexCount;

    i64 singleIndexSize = 0;
    switch (createInfo->indexType)
    {
        case MZNT_IndexType_U16: singleIndexSize = sizeof(u16); break;
        case MZNT_IndexType_U32: singleIndexSize = sizeof(u32); break;
        default:
            PNSLR_LogE(PNSLR_StringLiteral("Invalid index type"), PNSLR_GET_LOC());
            FORCE_DBG_TRAP;
            break;
    }

    i64 indexBufferSize = singleIndexSize * createInfo->indexCount;

    MZNT_VulkanMesh* output = PNSLR_New(MZNT_VulkanMesh, renderer->parent.allocator, PNSLR_GET_LOC(), nil);
    output->parent.type      = MZNT_RendererType_Vulkan;
    ...
    MZNT_INTERNAL_VK_CHECKED_CALL(vmaCreateBuffer(renderer->vmaAllocator,
    &(VkBufferCreateInfo)
    {
    ...
    &(VmaAllocationCreateInfo)
    {
        ...
        .usage = VMA_MEMORY_USAGE_AUTO,
    }, &(output->meshBuffer), &(output->meshAllocation), nil));
